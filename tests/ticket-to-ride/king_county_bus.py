...
class KingCountyBus:
    ...
    def _load_map_data(self):
        with open("bus_stops.txt", "r", encoding="utf-8") as stop_file:
            stop_data = json.loads(stop_file.read())
            for stop in stop_data:
                self.finder.add_node(stop["stop_id"], stop["stop_name"], (stop["x"], stop["y"]))
        
        with open("bus_edges.txt", "r", encoding="utf-8") as edge_file:
